...
export class Suggest<T> {
	...
	setSelectedItem(selectedIndex: number, scrollIntoView: boolean) {
		...
		const currentSelected = this.suggestions[normalizedIndex];

		prevSelected?.removeClass("is-selected");
		currentSelected?.addClass("is-selected");

		this.selectedItem = normalizedIndex;

		if (scrollIntoView) {
			currentSelected.scrollIntoView(false);
		}
	}
}

const normalizeIndex = (unnormalizedIndex: number, size: number): number => {
	return ((unnormalizedIndex % size) + size) % size;
};
